...
| **Ledger TTL / "bump"** | Soroban storage isn't permanent by default — each entry has a time-to-live measured in ledgers, and has to be periodically extended ("bumped") or it gets archived. | The contract extends instance TTL during initialization, event TTL when an event is created, and ticket TTL whenever `save_ticket` writes a ticket. See [the storage layout](docs/ARCHITECTURE.md#storage-layout) and `LEDGER_BUMP`/`LEDGER_THRESHOLD` in `contracts/ticketing/src/lib.rs`. |
